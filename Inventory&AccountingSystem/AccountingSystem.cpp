...
void AccountingSystem::generateReport()
{
    ...
    for (const Transaction &transaction : transactions)
    {
        ...
        std::cout << "  - " << transaction.getItem().getName() << ": " << transaction.getAmount() << " @ $" << transaction.getItem().getCost() << "\n";
        //}

        if (transaction.getType() == TransactionType::PURCHASE)
        {
            std::cout << "Total Cost: $" << transaction.getAmount() << "\n";
        }

        std::cout << "-----------------\n";
    }

    std::cout << "Current Balance: $" << getBalance() << "\n";
}
